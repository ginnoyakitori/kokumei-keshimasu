...
            } else {
                // 新規登録処理 (新規ユーザー)
                // cleared_idsはJSON型/TEXT型として '[]'を保存
...
                `SELECT ${clearedIdsColumn} FROM players WHERE id = $1 FOR UPDATE`, // FOR UPDATEでロック
                [playerId]
            );
            
...
            }
            
            const clearedIdsJson = checkResult.rows[0][clearedIdsColumn];
...
                // ★修正3-2: array_append()をやめ、Node.jsで加工したJSON文字列をそのまま更新する★
                `UPDATE players SET ${clearCountColumn} = ${clearCountColumn} + 1, ${clearedIdsColumn} = $2 WHERE id = $1 RETURNING ${clearCountColumn}`,
